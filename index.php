...
<div class="container">
    <div class="produto-grid row d-flex justify-content-center">
        <div class="produto col-xl-2 offset-xl-1 col-lg-3 offset-lg-1 col-md-4 offset-md-1 col-sm-4 offset-sm-1 col-6 offset-1 recentes mt-4 rounded border border-secondary" v-for="produto in produtos" :key="produto.id">
            ...
            <div class="row pl-2 pr-2">
                <h6><img src="imagens/{{produto.foto_usuario}}" style="width: 15%" class="rounded-circle"> {{produto.nome_usuario}}</h6>
            </div>
  
            <p><button v-if='checkCurtida(produto.id_produto)' @click="addLike(produto)" class="btn btn-info p-1">Gostei</button><strong>{{produto.likes}}</strong></p>
            <p><button v-if='checkCurtida(produto.id_produto)' @click="subLike(produto)" class="btn btn-danger p-1">Não gostei</button><strong>{{produto.deslikes}}</strong></p>
        </div>
    </div>
</div>
