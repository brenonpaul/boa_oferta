...
id_estado int,
CONSTRAINT fk_cidade_id_estado FOREIGN KEY(id_estado) REFERENCES estados (id_estado)
);
...
CONSTRAINT fk_bairro_id_cidade FOREIGN KEY(id_cidade) REFERENCES cidades (id_cidade)
);
...
CONSTRAINT fk_endereco_id_bairro FOREIGN KEY(id_bairro) REFERENCES bairros (id_bairro)
);
...
CONSTRAINT fk_mercados_id_rua FOREIGN KEY(id_rua) REFERENCES ruas (id_rua)
);
...
preco varchar(25),
CONSTRAINT fk_produto_cpf FOREIGN KEY(cpf) REFERENCES usuarios (cpf)
);
...
INSERT INTO bairros (nome_bairro,id_cidade) Values
...
INSERT INTO mercados (id_rua, nome_mercado) Values 
